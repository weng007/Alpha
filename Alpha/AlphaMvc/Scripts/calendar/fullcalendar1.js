/*!
 * fullCalendar1 v3.1.0
 * Docs & License: http://fullCalendar1.io/
 * (c) 2016 Adam Shaw
 */
...
$.fn.fullCalendar1 = function(options) {
	...
	this.each(function(i, _element) { // loop each DOM element involved
		...
		var calendarManPower = element.data('fullCalendar1'); // get the existing calendar object (if any)
		var singleRes; // the returned value of this single method call

		// a method call
		if (typeof options === 'string') {
		    if (calendarManPower && $.isFunction(calendarManPower[options])) {
		        singleRes = calendarManPower[options].apply(calendarManPower, args);
				if (!i) {
		...
		else if (!calendarManPower) { // don't initialize twice
		    calendarManPower = new CalendarManPower(element, options);
		    element.data('fullCalendar1', calendarManPower);
			calendarManPower.render();
		}
	});
...
/* fullCalendar1-specific DOM Utilities
----------------------------------------------------------------------------------------------------------------------*/
...
/* fullCalendar1-specific Misc Utilities
----------------------------------------------------------------------------------------------------------------------*/
...
momentProperties.push('_fullCalendar1');
...
newMomentProto.week = newMomentProto.weeks = function(input) {
	var weekCalc = this._locale._fullCalendar1_weekCalc;
...
newMomentProto.time = function(time) {
	...
	// Fallback to the original method (if there is one) if this moment wasn't created via fullCalendar1.
...
var Grid = FC.Grid = Class.extend(ListenerMixin, MouseIgnorerMixin, {
	...
	fabricateHelperEvent: function(eventLocation, sourceSeg) {
		...
		this.view.calendarManPower.normalizeEventDates(fakeEvent);
	...
	computeSelection: function(span0, span1) {
		...
		if (span && !this.view.calendarManPower.isSelectionSpanAllowed(span)) {
			return false;
		}
	...
	*/
	...
	getDayClasses: function(date, noThemeHighlight) {
		var view = this.view;
		var today = view.calendarManPower.getNow();
...
Grid.mixin({
	...
	buildBusinessHourEvents: function(wholeDay, businessHours) {
	    var calendarManPower = this.view.calendarManPower;
		var events;

		if (businessHours == null) {
			// fallback
			// access from calendawr. don't access from view. doesn't update with dynamic options.
		    businessHours = calendarManPower.options.businessHours;
		}

		events = calendarManPower.computeBusinessHourEvents(wholeDay, businessHours);
	...
	buildSegDragListener: function(seg) {
		var _this = this;
		var view = this.view;
		var calendarManPower = view.calendarManPower;
		var el = seg.el;
		var event = seg.event;
		var isDragging;
		...
		var dragListener = this.segDragListener = new HitDragListener(view, {
			...
			hitOver: function(hit, isOrig, origHit) {
				...
				if (dropLocation && !calendarManPower.isEventSpanAllowed(_this.eventToSpan(dropLocation), event)) {
					disableCursor();
					dropLocation = null;
				}
		...
		if (dragStart.hasTime() === dragEnd.hasTime()) {
			...
			if (event.allDay && durationHasTime(delta)) {
				dropLocation = {
					...
					end: calendarManPower.getEventEnd(event), // will be an ambig day
					allDay: false // for normalizeEventTimes
				};
				calendarManPower.normalizeEventTimes(dropLocation);
			}
	...
	listenToExternalDrag: function(el, ev, ui) {
		...
		var dragListener = _this.externalDragListener = new HitDragListener(this, {
			...
			hitOver: function(hit) {
				...
				if ( // invalid hit?
					dropLocation &&
					!calendarManPower.isExternalSpanAllowed(_this.eventToSpan(dropLocation), dropLocation, meta.eventProps)
				) {
					disableCursor();
					dropLocation = null;
				}
	...
	computeExternalDrop: function(span, meta) {
	    var calendarManPower = this.view.calendarManPower;
		var dropLocation = {
		    start: calendarManPower.applyTimezone(span.start), // simulate a zoned event start date
			end: null
		};
	...
	buildSegResizeListener: function(seg, isStart) {
		...
		var el = seg.el;
		var event = seg.event;
		var eventEnd = calendarManPower.getEventEnd(event);
		var isDragging;
		...
		var dragListener = this.segResizeListener = new HitDragListener(this, {
			...
			hitOver: function(hit, isOrig, origHit) {
				...
				if (resizeLocation) {
				    if (!calendarManPower.isEventSpanAllowed(_this.eventToSpan(resizeLocation), event)) {
						disableCursor();
						resizeLocation = null;
					}
	...
	computeEventResize: function(type, startSpan, endSpan, event) {
		...
		resizeLocation = {
			start: event.start.clone(),
			end: calendarManPower.getEventEnd(event),
			allDay: event.allDay
		};
		...
		if (resizeLocation.allDay && durationHasTime(delta)) {
			...
			calendarManPower.normalizeEventTimes(resizeLocation);
		}
		...
		if (!resizeLocation.start.isBefore(resizeLocation.end)) {
			...
			defaultDuration =
				...
				(event.allDay ?
					calendarManPower.defaultAllDayEventDuration :
					calendarManPower.defaultTimedEventDuration);
	...
	eventToRange: function(event) {
		...
		var end = (
				event.end ?
					...
					calendarManPower.getDefaultEventEnd(
						event.allDay != null ?
							event.allDay :
							!event.start.hasTime(),
						event.start
		...
		calendarManPower.localizeMoment(start);
		calendarManPower.localizeMoment(end);
...
// Require all HTML5 data-* attributes used by fullCalendar1 to have this prefix.
// A value of '' will query attributes like data-event. A value of 'fc' will query attributes like data-fc-event.
FC.dataAttrPrefix = '';

// Given a jQuery element that might represent a dragged fullCalendar1 event, returns an intermediate data structure
...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderNumberCellHtml: function(date) {
		...
		if (this.view.cellWeekNumbersVisible) {
			...
			if (date._locale._fullCalendar1_weekCalc === 'ISO') {
		...
		return $('<a class="fc-more"/>')
			...
			.on('click', function(ev) {
				...
				if (clickOption === 'popover') {
				    ...
				    view.calendarManPower.zoomTo(date, clickOption);
				}
			});
	},
...
var View = FC.View = Class.extend(EmitterMixin, ListenerMixin, {
	...
	calendarManPower: null, // owner Calendar object
	...
	constructor: function (calendarManPower, type, options, intervalDuration) {

	    this.calendarManPower = calendarManPower;
	...
	publiclyTrigger: function(name, thisObj) { // arguments beyond thisObj are passed along
	    var calendarManPower = this.calendarManPower;

	    return calendarManPower.publiclyTrigger.apply(
			calendarManPower,
	...
	computeRange: function(date) {
		...
		if (/year|month|week|day/.test(intervalUnit)) { // whole-days?
			...
			if (!intervalStart.hasTime()) {
			    intervalStart = this.calendarManPower.time(0); // give 00:00 time
			}
			if (!intervalEnd.hasTime()) {
			    intervalEnd = this.calendarManPower.time(0); // give 00:00 time
			}
		}
	...
	updateTitle: function() {
		...
		this.calendarManPower.setToolbarsTitle(this.title);
	},
		...
		return this.formatRange(
			{
			    ...
			    start: this.calendarManPower.applyTimezone(this.intervalStart),
			    end: this.calendarManPower.applyTimezone(this.intervalEnd)
			},
	...
	startNowIndicator: function() {
		...
		if (this.opt('nowIndicator')) {
			...
			if (unit) {
				...
				this.initialNowDate = this.calendarManPower.getNow();
	...
	updateHeight: function(isResize) {
	    var calendarManPower = this.calendarManPower; // we poll the calendar for height information

		this.setHeight(
			calendarManPower.getSuggestedViewHeight(),
			calendarManPower.isHeightAuto()
		);
	},
	...
	freezeHeight: function() {
	    this.calendarManPower.freezeContentHeight();
	},


	thawHeight: function() {
	    this.calendarManPower.thawContentHeight();
	},
	...
	bindEvents: function() {
		...
		if (!this.isEventsBound) {
			...
			this.rejectOn('eventsUnbind', this.requestEvents()).then(function(events) { // TODO: test rejection
			    _this.listenTo(_this.calendarManPower, 'eventsReset', _this.setEvents);
	...
	unbindEvents: function() {
		if (this.isEventsBound) {
			...
			this.stopListeningTo(this.calendarManPower, 'eventsReset');
			this.unsetEvents();
	...
	requestEvents: function() {
	    return this.calendarManPower.requestEvents(this.start, this.end);
	},


	getCurrentEvents: function() {
	    return this.calendarManPower.getPrunedEventCache();
	},
	...
	reportEventDrop: function(event, dropLocation, largeUnit, el, ev) {
	    var calendarManPower = this.calendarManPower;
	    var mutateResult = calendarManPower.mutateEvent(event, dropLocation, largeUnit);
		var undoFunc = function() {
			mutateResult.undo();
			calendarManPower.reportEventChange();
		};

		this.triggerEventDrop(event, mutateResult.dateDelta, undoFunc, el, ev);
		calendarManPower.reportEventChange(); // will rerender events
	},
	...
	reportExternalDrop: function(meta, dropLocation, el, ev, ui) {
		...
		if (eventProps) {
			...
			event = this.calendarManPower.renderEvent(eventInput, meta.stick)[0]; // renderEvent returns an array
		}
	...
	reportEventResize: function(event, resizeLocation, largeUnit, el, ev) {
	    var calendarManPower = this.calendarManPower;
	    var mutateResult = calendarManPower.mutateEvent(event, resizeLocation, largeUnit);
		var undoFunc = function() {
			mutateResult.undo();
			calendarManPower.reportEventChange();
		};
	...
	triggerSelect: function(span, ev) {
		this.publiclyTrigger(
			'select',
			null,
			this.calendarManPower.applyTimezone(span.start), // convert to calendar's tz for external API
			this.calendarManPower.applyTimezone(span.end), // "
			ev
		);
	},
	...
	triggerDayClick: function(span, dayEl, ev) {
		this.publiclyTrigger(
			'dayClick',
			dayEl,
			this.calendarManPower.applyTimezone(span.start), // convert to calendar's timezone for external API
			ev
		);
	},
...
function Toolbar(calendarManPower, toolbarOptions) {
	var t = this;
	...
	function render() {
		...
		tm = calendarManPower.options.theme ? 'ui' : 'fc';
			...
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
					    ...
					    if ((customButtonProps = (calendarManPower.options.customButtons || {})[buttonName])) {
					    ...
					    else if ((viewSpec = calendarManPower.getViewSpec(buttonName))) {
							buttonClick = function() {
							    calendarManPower.changeView(buttonName);
							};
							viewsWithButtons.push(buttonName);
							overrideText = viewSpec.buttonTextOverride;
							defaultText = viewSpec.buttonTextDefault;
						}
					    else if (calendarManPower[buttonName]) { // a calendar method
							buttonClick = function() {
							    calendarManPower[buttonName]();
							};
							overrideText = (calendarManPower.overrides.buttonText || {})[buttonName];
							defaultText = calendarManPower.options.buttonText[buttonName]; // everything else is considered default
						}
						...
						if (buttonClick) {
							...
							themeIcon =
								customButtonProps ?
									...
									calendarManPower.options.themeButtonIcons[buttonName];

							normalIcon =
								customButtonProps ?
									customButtonProps.icon :
									calendarManPower.options.buttonIcons[buttonName];

							if (overrideText) {
								innerHtml = htmlEscape(overrideText);
							}
							else if (themeIcon && calendarManPower.options.theme) {
								innerHtml = "<span class='ui-icon ui-icon-" + themeIcon + "'></span>";
							}
							else if (normalIcon && !calendarManPower.options.theme) {
...
var calendarManPower = FC.calendarManPower = Class.extend({

	dirDefaults: null, // option defaults related to LTR or RTL
	localeDefaults: null, // option defaults related to current locale
	overrides: null, // option overrides given to the fullCalendar1 constructor
	...
	constructor: calendarManPower_constructor,
	...
	populateOptionsHash: function() {
		...
		if (!localeDefaults) { // explicit locale option not given or invalid?
		    locale = calendarManPower.defaults.locale;
			localeDefaults = localeOptionHash[locale] || {};
		}
		...
		isRTL = firstDefined( // based on options computed so far, is direction RTL?
			...
			calendarManPower.defaults.isRTL
		);
		dirDefaults = isRTL ? calendarManPower.rtlDefaults : {};

		this.dirDefaults = dirDefaults;
		this.localeDefaults = localeDefaults;
		this.options = mergeOptions([ // merge defaults and overrides. lowest to highest precedence
			calendarManPower.defaults, // global defaults
			dirDefaults,
			localeDefaults,
			this.overrides,
	...
	buildViewSpecButtonText: function(spec, requestedViewType) {
		...
		spec.buttonTextDefault =
			...
			queryButtonText(calendarManPower.defaults) ||
...
calendarManPower.mixin(EmitterMixin);


function calendarManPower_constructor(element, overrides) {
	var t = this;
	...
	t.zoomTo = zoomTo;
	t.getDate = getDate;
	t.getCalendarManPower = getCalendarManPower;
	t.getView = getView;
	...
	t.bindOptions([
		...
		if ( // whitelist certain kinds of input
			...
			localeData._fullCalendar1_weekCalc = weekNumberCalculation; // moment-ext will know what to do with it
		}
...
CalendarManPower.englishDefaults = { // used by locale.js
	dayPopoverFormat: 'dddd, MMMM D'
};


CalendarManPower.rtlDefaults = { // right-to-left defaults
...
// TODO: document the structure and ordering of a fullCalendar1 locale file
...
FC.datepickerLocale = function(localeCode, dpLocaleCode, dpOptions) {
	...
	// get the fullCalendar1 internal option hash for this locale. create if necessary
	...
	if ($.datepicker) {
		...
		// fullCalendar1 and MomentJS use locale codes like "pt-br" but Datepicker
...
// Sets fullCalendar1-specific translations. Will set the locales as the global default.
FC.locale = function(localeCode, newFcOptions) {
	...
	// set it as the default locale for fullCalendar1
	CalendarManPower.defaults.locale = localeCode;
};
...
FC.locale('en', CalendarManPower.englishDefaults);
...
CalendarManPower.prototype.normalizeEvent = function(event) {
};


// Does the given span (start, end, and other location information)
// fully contain the other?
CalendarManPower.prototype.spanContainsSpan = function (outerSpan, innerSpan) {
...
CalendarManPower.prototype.getPeerEvents = function (span, event) {
...
CalendarManPower.prototype.isEventSpanAllowed = function (span, event) {
	var source = event.source || {};
...
CalendarManPower.prototype.isExternalSpanAllowed = function (eventSpan, eventLocation, eventProps) {
	var eventInput;
	var event;
...
CalendarManPower.prototype.isSelectionSpanAllowed = function (span) {
...
CalendarManPower.prototype.isSpanAllowed = function (span, constraint, overlap, event) {
...
CalendarManPower.prototype.constraintToEvents = function (constraintInput) {
...
CalendarManPower.prototype.eventIntersectsRange = function (event, range) {
...
CalendarManPower.prototype.getCurrentBusinessHourEvents = function (wholeDay) {
	return this.computeBusinessHourEvents(wholeDay, this.options.businessHours);
};

// Given a raw input value from options, return events objects for business hours within the current view.
CalendarManPower.prototype.computeBusinessHourEvents = function (wholeDay, input) {
	if (input === true) {
...
CalendarManPower.prototype.expandBusinessHourEvents = function (wholeDay, inputs, ignoreNoDow) {
