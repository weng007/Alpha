    ...
    $.ajax(
    {
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.Table.length; i++) {
                ...
                html += '<a href="/JobOrder/EditJobOrder?id=' + data.Table[i].ID + '&' + "CarlendarJob" + '" id="edit' + data.Table[i].ID + '" style="margin-right: 3px;">' + '<img src="/Images/edit.png" class="imgJobCalendarUpdate"/></a>';
                html += '<a href="#" id="del' + data.Table[i].ID + '" onclick="ConfirmDialog(' + " 'Delete'" + ',' + "'IncomeMaster'" + ',' + data.Table[i].ID + ')" style="margin-right: 5px; Display:none;" >' + '<img src="/Images/delete.png" class="imgJobCarlendarDelete"/></a>';
                html += '<a href="/JobOrder/EditJobOrder?id=' + data.Table[i].ID + '&' + "CarlendarJob" + '&' + "true" + '" id="read' + data.Table[i].ID + '">' + '<img src="/Images/view.png" class="imgJobCalendarView" /></a>';
                html += '</td>';
                html += '</tr>';
                //});
            }
