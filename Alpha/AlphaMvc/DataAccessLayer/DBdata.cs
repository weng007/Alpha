...
namespace AlphaMVC.DataAccessLayer
{
    public class DBdata
    {
        ...
        public DataSet GetRptWageTechnician(string ID,DateTime FromManDate,DateTime ToManDate)
        {
            using (SqlConnection conObj = new SqlConnection(conStr))
            {
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("@FromManDate", FromManDate);
                    cmd.Parameters.AddWithValue("@ToManDate", ToManDate);
                    conObj.Open();
                    SqlDataAdapter da = new SqlDataAdapter();
                    da.SelectCommand = cmd;
                    ds = new DataSet();
        ...
        public DataSet GetRptJobPayment(string ID, DateTime FromManDate, DateTime ToManDate)
        {
            using (SqlConnection conObj = new SqlConnection(conStr))
            {
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SP_RptJobPayment", conObj);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@ID", ID);
                    cmd.Parameters.AddWithValue("@FromManDate", FromManDate);
                    cmd.Parameters.AddWithValue("@ToManDate", ToManDate);
